...
export class PasswordResetPageComponent implements OnInit {
	...
	resetPassword() {
		...
		this.userService.resetPassword(this.queryParams.user, this.queryParams.token, this.passwordData.new)
			.subscribe(
				(res: any) => {
					this.status = 'success'
					this.statusMessage = 'Password successfully reset! Redirecting to login page...'
					setTimeout(() => {
				...
				(err: any) => {
					...
					this.status = 'warn'
					this.statusMessage = 'Password reset failed. Your reset token may be incorrect or may have expired. ' +
	...
	sendPasswordResetEmail() {
		this.isFetching = true
		this.userService.sendPasswordResetEmail(this.emailData)
			.subscribe(
				(res: any) => {
					this.isFetching = false
				...
				(err: any) => {
					this.status = 'warn'
					this.statusMessage = 'Failed to send password reset email. Try again later or contact an administrator.'
					console.error(err.error.message, err.error)
				})
	}
