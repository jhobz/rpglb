...
export class AdminPageComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        if (
            !this.user ||
            !this.user.roles ||
            (!this.user.roles.includes("safety") &&
    ...
    getUsers() {
        this.userService.getUsers({ limit: 100000 }).subscribe(
            (users: User[]) => {
                this.userList = users
                this.attendees = this.userList.filter((u) =>
    ...
    updateSpeedrunEvent(
        ...
        this.speedrunEventService
            .editSpeedrunEvent(this.speedrunEvent)
            .subscribe(
                (srEvent: SpeedrunEvent) => {
                    this.snackBar.open(
                        `${readableName} set to "${readableValue}" successfully!`,
                        "",
                        {
                            duration: 5000,
    ...
    updateState(property: string, value: any) {
        switch (property) {
            case "gameSubmissions":
                this.updateSpeedrunEvent(
                    "areGameSubmissionsOpen",
                    value,
                    "Game submissions",
                    value ? "open" : "closed"
                )
