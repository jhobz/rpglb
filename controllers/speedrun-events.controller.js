...
exports.getSpeedrunEvents = async function (req, res, next) {
	...
	try {
		let speedrunEvents = await SpeedrunEventService.getSpeedrunEvents({}, page, limit)
...
exports.createSpeedrunEvent = async function (req, res, next) {
	...
	try {
		let createdSpeedrunEvent = await SpeedrunEventService.createSpeedrunEvent(speedrunEvent)
...
exports.updateSpeedrunEvent = async function (req, res, next) {
	...
	if (!req.user || !req.user._id) {
		console.warn('Unauthorized speedrun event editing attempted', req.user, req.body)
		return res.status(401).json( {
			status: 401,
			message: 'Unauthorized. You must be logged in to edit a speedrun event.'
		} )
	}
	...
	if (!userRoles ||
		...
		return res.status(401).json( {
			status: 401,
			message: 'Unauthorized. You do not have permission to edit this speedrun event.'
		} )
	}
	...
	try {
		let updatedSpeedrunEvent = await SpeedrunEventService.updateSpeedrunEvent(speedrunEvent)
		return res.status(200).json( {
			status: 200,
...
exports.removeSpeedrunEvent = async function (req, res, next) {
	if (!req.user || !req.user.roles || !req.user.roles.includes('admin')) {
		console.warn('Unauthorized speedrun event deletion attempted', req.user, req.body)
		return res.status(401).json( {
			status: 401,
			message: 'Unauthorized. You do not have permission to delete this speedrun event.'
		} )
	}
...
exports.getSpeedrunEvent = async function (req, res, next) {
	try {
		let speedrunEvent = await SpeedrunEventService.getSpeedrunEventById(req.params.id)
